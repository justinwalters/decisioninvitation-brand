...
def mark(x,y,scale=1,body='register',point='orange',internal='periwinkle',background='optic'):
    c.setStrokeColor(C[body]); c.setLineWidth(20*scale)
    for upper in (True,False):
        p=c.beginPath(); p.moveTo(x+34*scale,H-(y+(40 if upper else 216)*scale)); p.lineTo(x+92*scale,H-(y+(40 if upper else 216)*scale))
        if upper: p.curveTo(x+143*scale,H-(y+40*scale),x+181*scale,H-(y+76*scale),x+181*scale,H-(y+128*scale))
        else: p.curveTo(x+143*scale,H-(y+216*scale),x+181*scale,H-(y+180*scale),x+181*scale,H-(y+128*scale))
    ...
    for sy,c1y,c2y in ((84,84,101),(128,128,128),(172,172,155)):
        r=c.beginPath(); r.moveTo(x+34*scale,H-(y+sy*scale)); r.lineTo(x+100*scale,H-(y+sy*scale))
        r.curveTo(x+132*scale,H-(y+c1y*scale),x+158*scale,H-(y+c2y*scale),x+181*scale,H-(y+128*scale))
        c.drawPath(r,fill=0,stroke=1)
    c.setFillColor(C[point]); c.circle(x+216*scale,H-(y+72*scale),12*scale,fill=1,stroke=0)

def convergence_motif(x,y,dark=False):
    """Echo the five-input D without drawing its absent vertical spine."""
    body='white' if dark else 'register'; internal='periwinkle' if dark else 'cobalt'
    c.setStrokeColor(C[body]); c.setLineWidth(8)
    for sy,c1y,c2y in ((0,0,18),(144,144,126)):
        p=c.beginPath(); p.moveTo(x,H-(y+sy)); p.lineTo(x+54,H-(y+sy))
        p.curveTo(x+94,H-(y+c1y),x+126,H-(y+c2y),x+150,H-(y+72))
        c.drawPath(p,fill=0,stroke=1)
    c.setStrokeColor(C[internal]); c.setLineWidth(2.5)
    for sy,c1y,c2y in ((36,36,54),(72,72,72),(108,108,90)):
        p=c.beginPath(); p.moveTo(x,H-(y+sy)); p.lineTo(x+62,H-(y+sy))
        p.curveTo(x+96,H-(y+c1y),x+126,H-(y+c2y),x+150,H-(y+72))
        c.drawPath(p,fill=0,stroke=1)
    c.setFillColor(C['orange']); c.circle(x+183,H-(y+72),5,fill=1,stroke=0)
...
('05 / Logo idea','The Multipath D.','Recognition through convergence.','Five independent inputs begin on one invisible axis and resolve at a single decision point. The aligned openings let the eye complete the missing D spine. A separate lowercase i carries one orange provenance dot.'),
...
for n,(section,title,deck,body) in enumerate(PAGES,2):
    ...
    if n==6: mark(668,275,.75,'white' if dark else 'register','orange','periwinkle','register' if dark else 'optic')
        ...
        convergence_motif(654,306,dark)
c.save()
